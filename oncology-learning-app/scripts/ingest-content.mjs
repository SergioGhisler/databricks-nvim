...
function parseBlocks(text) {
  ...
  for (const b of blocks) {
    if (b.startsWith('TOPIC:')) {
      ...
      quizBank.push({ id: `q-${quizIndex++}`, topicId: topics[0]?.id || 'topic-1', type: 'mcq', question, options, answer, explanation })
      ...
      const explanation = (lines.find((l) => l.startsWith('EXPLAIN:')) || 'EXPLAIN:').replace('EXPLAIN:', '').trim()
      quizBank.push({ id: `q-${quizIndex++}`, topicId: topics[0]?.id || 'topic-1', type: 'short', question, answerKeywords: keywords, explanation })
